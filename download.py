...
def main():
    """
    Download folder content from google drive without zipping.
    """
    ...
    print(colored('* Directory to save - by default it will be the current directory', 'blue'))
    location = input("   - Path: ")
    while not location:
        location = input("   - Path: ")
...
def download_file(service, file_id, location, filename):
    ...
    while done is False:
        ...
        if status:
            print(int(status.progress() * 100), " percent complete         \r", end='', flush=True)
    print("")
